...
@Component({
  selector: 'app-govassets',
  templateUrl: './govassets.component.html',
  styleUrls: ['./govassets.component.scss']
})
export class GovassetsComponent implements OnInit{
  ...
  governorateAssets() {
   ...
   // this.assetDetailService.PyramidGovernorateChart()
    ...
    .PyramidGovernorateChartByParams(this.filterAassetCountParam) 
    .subscribe(orgChart => {
      
      orgChart.forEach(element => {
        dataList.push({
          "governorateName": element["governorateName"],
          "governorateNameAr": element["governorateNameAr"], "count": element["count"], "population": element["population"]
        });
      });
      this.triangleChartOptions = {
        ...
        plotOptions: {
          bar: {
            ...
            horizontal: true,
            distributed: true,
            barHeight: "100%",//((dataList.map(a =>a.population)/100)*100).toString(),
            isFunnel: true
          }
        },
        colors: [
